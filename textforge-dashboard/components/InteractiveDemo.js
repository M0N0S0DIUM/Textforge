...
const ACTIONS = {
  ...
  trimtext: (text) => text.trim(),
  pascalcase: (text) => text.split(/[\s_-]+/).map(p => p.charAt(0).toUpperCase() + p.slice(1).toLowerCase()).join(''),
  hash: (text) => 'hash_' + text.length,
  random: (text) => Math.random().toString(36).substring(7),
  palindromecheck: (text) => ({ isPalindrome: text === text.split('').reverse().join(''), normalizedText: text.toLowerCase().replace(/[^a-z0-9]/g, '') }),
